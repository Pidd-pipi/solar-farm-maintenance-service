...
var opsSearchBuffer []OpsRecord

// opsFilterToBuffer filters items into a shared package-level buffer.
func opsFilterToBuffer(items []OpsRecord, query OpsQuery) []OpsRecord {
	opsSearchBuffer = opsSearchBuffer[:0]
	for _, item := range items {
		if opsMatch(item, query) {
			opsSearchBuffer = append(opsSearchBuffer, item)
		}
	}
	return opsSearchBuffer
}
