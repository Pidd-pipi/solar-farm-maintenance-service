...
func checkRecordCompliance(record OpsRecord, rules []OpsRule) (*ComplianceReport, error) {
	...
	for _, rule := range rules {
		...
		if result.Passed && rule.Terminal && record.Status != OpsStatusClosed {
			result.Passed = false
			result.Reason = "terminal rule requires closed status"
		}
		if result.Passed {
			report.Passed++
		} else {
...
func compliancePassRate(report *ComplianceReport) int {
	...
	return report.Passed * 100 / (report.Passed + report.Failed)
}
