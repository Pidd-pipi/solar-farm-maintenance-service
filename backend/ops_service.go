...
func (s *OpsService) Search(ctx context.Context, q OpsQuery) (OpsPage, error) {
	...
	filtered := opsFilterToBuffer(items, q)
...
// ApplyLabel updates one label on a record through the store.
func (s *OpsService) ApplyLabel(ctx context.Context, id, key, value string) error {
	return s.store.TouchLabels(ctx, id, key, value)
}
